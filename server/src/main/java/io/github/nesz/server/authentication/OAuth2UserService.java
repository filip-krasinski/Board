...
import io.github.nesz.server.authentication.info.OAuth2UserInfo;
import io.github.nesz.server.authentication.info.OAuth2UserInfoFactory;
...
public class OAuth2UserService extends DefaultOAuth2UserService {
    ...
    OAuth2UserService(UserRepository userRepository) {
    ...
    private OAuth2User processOAuth2User(OAuth2UserRequest oAuth2UserRequest, OAuth2User oAuth2User)
        ...
        /*
            Since github doesn't give us an email while authorizing if it is private,
            we have to make an additional request with access token in order to get it.
         */
        if (oAuth2UserInfo.getEmail() == null || oAuth2UserInfo.getEmail().isEmpty()) {
            if (!oAuth2UserRequest.getClientRegistration().getRegistrationId().equalsIgnoreCase(AuthenticationProvider.github.name())) {
        ...
        if (userOptional.isEmpty()) {
            ...
            if (!user.getAuthProvider().equals(AuthenticationProvider.valueOf(registrationId))) {
        ...
        return UserDetails.create(user, oAuth2UserInfo.getAttributes());
    }
